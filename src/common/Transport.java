...
public class Transport {
	...
	public static <T extends Serializable> T receive(Socket socket) throws IOException, ClassNotFoundException {
		...
		//citim bucata cu bucata
		int count = 0;
		while(count < buffer.length) {
			count += socket.getInputStream().read(buffer, count, buffer.length); //citim din buffer de la poz recenta si pana la final 
	...
	public static <T extends Serializable> void send(T object, Socket socket) throws IOException {
		...
		socket.getOutputStream().write(buffer, 0, buffer.length); //scriem ce avem in buffer
		socket.getOutputStream().flush(); //flush pe zona tampon
	}
